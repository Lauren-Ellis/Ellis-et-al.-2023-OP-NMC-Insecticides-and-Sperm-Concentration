...
author: "Lauren Ellis"
date of last update: December 31, 2022 
...
R versions as of date of submission (12/31/2022):   

* R version 4.2.2 (2022-10-31) "Innocent and Trusting"    
* RStudio 2022.07.2+576 "Spotted Wakerobin" (from "About RStudio")    

Meta-analysis package versions as of date of submission (12/31/2022):   
...
```{r import data}
OPNMC_data <- read_excel("Ellis-et-al-2023-study-results-data.xlsx",
                          sheet = "Results Data")
OPNMC_data
```
...
* When calculating partial mean difference effect sizes from adjusted measures, it is recommended to use unadjusted group-level standard deviations (SDs) to calculate the pooled SD.   
    + If unadjusted SDs are unavailable, it is recommended to use a multiplier --- $sqrt(1-R^2)$ --- in an attempt to restore the unadjusted pooled SD (remove the effect of model fit).   
        - This concept is discussed in the context of ANCOVA models in [Borenstein & Hedges, 2019, Chp. 11](https://www.jstor.org/stable/10.7758/9781610448864) (p. 215-216) and is expanded for applications to linear regression models in [Aloe et al., 2021](https://www.tandfonline.com/doi/abs/10.1080/00220973.2021.1966605).  
...
### 1. Convert mean difference to Cohen's D   
**Unadjusted md/adjusted md/beta-md coefficient to Cohen's D**      
...
OPNMC_Data_MD <- OPNMC_data %>%
  ...
  mutate(
    Di = md2smd(md = MD, sd.e = SD_E, sd.c = SD_C, n.e = N_E, n.c = N_C),
...
* Used methods from [Borenstein & Hedges, 2019, Chp. 11](https://www.jstor.org/stable/10.7758/9781610448864) and [Aloe et al., 2021](https://www.tandfonline.com/doi/abs/10.1080/00220973.2021.1966605) to convert t-statistics into Cohen's D.  
...
Rules for calculating t-statistics (prior to import into R):   

* If p-value was available, calculated t-statistic from p-value and degrees of freedom    
* If p-value was not available, calculated t-statistic from beta and SE   
...
OPNMC_Data_Tstat_Neg <- OPNMC_data %>%
  ...
  filter(MD < 0) %>%
  mutate(
    Di = tstat2smd(t = (as.double(T_Stat)), n.e = N_E, n.c = N_C) * -1,
...
### 2. Convert odds ratio to Cohen's D  
Notes:    
...
* Unadjusted/adjusted ORs were accepted as inputs.   
...
or2smd_var <- function(or.ub, or.lb) {
 ((3 * (((log(or.ub) - log(or.lb)) / 3.92)^2)) / (pi^2))
} # Borenstein & Hedges, 2019; p. 232-233, Table 11.5 & 11.77

# Apply functions
OPNMC_Data_ORTable <- OPNMC_data %>%
  filter(Formula_Inputs == "OR") %>%
  mutate(
...
OPNMC_Data_BetaOR <- OPNMC_data %>%
  filter(Formula_Inputs == "Beta-OR") %>%
  mutate(
    Di = (or2smd(or = OR) * -1), # multiplied by -1 to get correct direction of effect
    D_Vi = or2smd_var(or.ub = OR_UB, or.lb = OR_LB)
  )
...
### 3. Convert correlation to Cohen's D
**Bivariate correlation to Cohen's D**    
...
OPNMC_Data_R <- OPNMC_data %>%
  ...
  mutate(
    ...
    Di = as.double(r2smd(r = Abs_R) * -1),  # multiplied by -1 to get correct direction of effect
    D_Vi = as.double(r2smd_var(r = Abs_R, r.v = R_Vi))
    )
```
...
* Used methods from [Aloe et al., 2013](https://www.journals.uchicago.edu/doi/abs/10.5243/jsswr.2013.24) to calculate a partial correlation from homogenized multivariate linear regression beta coefficients (see formulas below)
    + 't' is the statistic of the regression coefficient in the multiple regression model   
    + 'df' is the degrees of freedom
    + 'p' is the number predictors included in the model, including the exposure variable of interest   
        - "The only difference between the variances of the bivariate correlation and the partial correlation is the incorporation of 'p' in the denominator of the partial correlation."     
...
### 4. Merge calculated Cohen's D data
...
### 5. Convert Cohen's D to Hedges' G   

Notes:  

* Hedges' G refers to the bias-corrected Cohen's D, or standardized mean difference   
* Correction factor (J) from [Borenstein & Hedges, 2019, p. 213, Eq. 11.19](https://www.jstor.org/stable/10.7758/9781610448864): $J(df) = 1 - (3 / ((4*df) - 1))$    
    + In the context of bivariate statistics, df = n1 + n2 - 2 (comparing two independent groups)    
    + In context of ANCOVA, df should take into account of the number of covariates in the model, so df = n1 + n2 - 2 (number of groups) - q (number of covariates) (Borenstein & Hedges, 2019, p. 213)  
        - This idea was expanded from ANCOVA models to multivariate regression models (Beta-MD) in [Aloe et al., 2021](https://www.tandfonline.com/doi/abs/10.1080/00220973.2021.1966605):   
              $J(df) = (1 - (3/(4*((n1 + n2) - p - 1) -1)))$   
        - df = total sample size (n) - no. of predictors in model (p) - 1   
        - Similar to Borenstein formula for ANCOVA models, but 'p' includes the independent variable of interest whereas 'q' does not    
        - If bivariate, p = 1, if multivariate, p > 1   

```{r hedges g}
# Create functions
smd2hedgesG <- function(smd, N_Total, q, p) {
  smd * (1 - (3 / (4 * ((N_Total - p - 1) - 1) - p)))
}
...
`Total_Men_Included[1]` unique men were included in the primary meta-analysis based on the sum of the maximum amount of men included in the meta-analysis from each unique study population.   
...
Note: Ran meta-analyses for the full range of R options to examine the impact of potential misspecification of the assumed correlation between dependent effect sizes used to construct a variance-covariance matrix.
...
Note: Ran meta-analyses using the lower and upper bounds of the REML heterogeneity point estimates for each level of random effect.
...
See the 'Data Manipulations (SA1 & SA2)' tab of the extracted study results data for more detail.
...
Note: Did not include pooled effect estimates in the forest plot.
...
write_xlsx(forest_dat_nopooled,"forest_dat_nopooled.xlsx")
```
